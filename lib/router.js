  ...
  triggersEnter: [
    function() {
      if (!Meteor.user().admin) {
        return FlowRouter.go(FlowRouter.path('not_found'));
      }
    }
  ]
});
...
exposed.route("/", {
  ...
  subscriptions () {
    this.register('deepstreamsMostRecent', longTermSubs.subscribe('deepstreamsMostRecent'));
    this.register('deepstreamsEditorsPick', longTermSubs.subscribe('deepstreamsEditorsPick'));
  },
...
/* contest over
exposed.route("/contest", {
  name: "contest",
  action(){
    setTitle('$250 Contest');
    setMetaImage();
    setMetaDescription('We want to see what kind of stories YOU tell using DeepStreams! We are providing two prizes for the best DeepStreams created by 11:59 Pacific Time (PT) August 24');
    setStatusCode();
    return BlazeLayout.render("contest");
  },
  triggersEnter: [function(){
    $('html, body').scrollTop(0);
  }]
...
exposed.route("/watch/:userPathSegment/:streamPathSegment", {
  ...
  triggersEnter: [function(){
    ...
    Session.set('expandedPortraitCards', false);
  }]
});
...
admin.route('/', {
  name: 'admin_dash',
  action: function() {
    return BlazeLayout.render('admin_dashboard');
  },
  triggersEnter: [function(){
    $('html, body').scrollTop(0);
  }]
...
admin.route('/users', {
  name: 'admin_users',
  action: function() {
    return BlazeLayout.render('admin_users');
  },
  triggersEnter: [function(){
    $('html, body').scrollTop(0);
  }]
});
admin.route('/deepstreams', {
  name: 'admin_deepstreams',
  action: function() {
    return BlazeLayout.render('admin_deepstreams');
  },
  triggersEnter: [function(){
    $('html, body').scrollTop(0);
  }]
...
Accounts.onLogin(function() {
  if(Meteor.isClient){
    ...
    if (redirect != null) {
      if (redirect !== '/login') {
        Meteor.setTimeout(function(){ Session.set('redirectAfterLogin', false); },1500);
